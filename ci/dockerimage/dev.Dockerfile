...
RUN apk add --update --no-cache --simulate git composer build-base ${ALPINE_PHP_PACKAGE}-pecl-xdebug unzip vim | sha256sum >> /cachehash
...
RUN apk add --update --no-cache git composer build-base ${ALPINE_PHP_PACKAGE}-pecl-xdebug unzip vim
