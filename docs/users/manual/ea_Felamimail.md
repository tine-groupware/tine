...
<!--POP3-->
Was passiert nun mit der E-Mail, wenn sie beim Server abgeholt wurde? Sie landet als Eintrag im E-Mail-Client des Empfängers, damit er sie lesen kann. Auch hier unterscheidet man grundsätzlich zwei Verfahren:
...
Fangen wir bei den Navigations-Optionen der Tabelle an. Mit den Symbolen <img src="{{icon_url}}icon_arrow_back_last.svg" alt="drawing" width="16"/> und <img src="{{icon_url}}icon_arrow_action_last.svg" alt="drawing" width="16"/> navigieren Sie zur ersten und zur letzten Seite der Tabelle. Mit <img src="{{icon_url}}icon_arrow_left.svg" alt="drawing" width="16"/> und <img src="{{icon_url}}icon_arrow_right.svg" alt="drawing" width="16"/> können Sie in der Tabelle vor- und zurückwandern. Alternativ können Sie in dem Eingabebereich zwischen den Pfeilen die spezifische Tabellenseite eingeben, um schnell dorthin zu navigieren. Das <img src="{{icon_url}}icon_return.svg" alt="drawing" width="16"/>-Icon dient zum Neuladen der Tabelle.
...
!!! info "Wichtig"
    Behalten Sie diese Anzeige im Auge! Wenn Sie bei 100% steht, werden Ihnen keine weiteren E-Mails mehr zugestellt. Das ist insofern bedeutsam, als der Absender einer E-Mail an Sie darüber nicht immer informiert[^1] und stattdessen im Glauben gelassen wird, seine E-Mail habe Sie erreicht. Sie wiederum erhalten ebenfalls keine Benachrichtigung. Sollten Sie also übersehen, dass Ihr Speicher voll ist, sind Sie mit Ihrem Konto quasi offline, ohne es zu bemerken.
...
[^1]:
    Im Normalfall kommen E-Mails zum Empfänger mit einer Mitteilung wie "user over quota" o. ä. zurück. Gehen Sie jedoch davon aus, dass ein Nutzer das durchaus übersehen oder mit der Fehlermeldung nichts anfangen kann.

Weiter geht es mit der eigentlichen Tabelle.
Standardmäßig benennt der Tabellenkopf die wichtigsten Spalten; wie Sie mit dem kleinen Tabellenkopfsymbol rechts am äußeren Rand weitere Spalten zu- und abwählen, beschreibt [Allgemeine Hinweise zur Bedienung](ca_StandardBedienhinweise.md).
...
<img src="{{icon_url}}icon_email_forward.svg" alt="drawing" width="16"/>: Sie haben diese E-Mail an andere Empfänger weitergeleitet.
...
Kommen wir zum Reiter IMAP: Die Maske enthält als Pflichteingabefeld eine Bezeichnung für den Host. Damit ist die Bezeichnung des Post__eingangs__servers gemeint. Eine typische Angabe lautet zum Beispiel `imap.example.com`.
...
Damit haben wir alle für die reibungslose Funktion eines E-Mail-Kontos erforderlichen Einstellungen besprochen.
Unter dem Reiter Sieve finden Sie die Parameter für den Zugang zum E-Mail-Server Ihres Kontos, über den Sie sogenannte "Sieve"-Befehle übermitteln. "Sieve" ist eine Programmiersprache für Befehle an den E-Mail-Server, etwa zum Abschicken einer Abwesenheitsnotiz oder für Filterregeln. Dazu kommen wir im nächsten Abschnitt.
...
Kommen wir nun zu einigen Funktionen, die nicht zum Bearbeitungsmenü gehören. Sie erreichen sie nur über das sog. Kontextmenü, also wenn Sie mit dem Mauszeiger per _rechter_ Maustaste auf das E-Mail-Konto oder den Ordner klicken.

Beginnen wir mit dem Kontextmenü des E-Mail-Kontos.
Hier haben wir die Optionen Ordner hinzufügen, Ordnerliste aktualisieren, Abwesenheitsnotiz bearbeiten, Filterregeln bearbeiten, Benachrichtigungen, Konto bearbeiten und Konto löschen. Gehen wir diese nun einmal der Reihenfolge nach durch.

!!! note "Anmerkung"
    Alternativ können Sie von der Admin-Anwendung aus E-Mail-Kontos bearbeiten. Hierzu öffnen Sie die Admin-Anwendung und Navigieren zu E-Mail-Accounts. Dorf finden Sie eine Übersicht aller aktiven E-Mail-Konten.
...
bietet Ihnen rechts die Möglichkeit, aus den für dieses E-Mail-Konto angelegten Unterordnern einen auszuwählen.
</div>

E-Mail an folgende Adresse weiterleiten
<div style="margin-left: 40px">
Geben Sie rechts eine gültige E-Mail-Adresse an, an die weitergeleitet werden soll.
</div>

E-Mail mit folgendem Text ablehnen
<div style="margin-left: 40px">
Schreiben Sie den entsprechenden Text rechts in das Feld. 
</div>

E-Mail verwerfen
<div style="margin-left: 40px">
verwirft die E-Mail kommentarlos, als sei sie nicht angekommen. Der Absender erhält darüber auch keine Mitteilung. </div>

Nachricht mit folgendem Text beantworten
<div style="margin-left: 40px">
...
Mit dem Menüpunkt Konto bearbeiten rufen Sie die gleiche Bearbeitungsmaske wie mit der oben besprochenen Funktion Konto hinzufügen auf, nur dass sich hier die entsprechenden Daten des Kontos bereits in den Feldern befinden und geändert werden können.
...
<div style="margin-left: 40px">
markiert in einem Arbeitsgang alle in dem betreffenden Ordner befindlichen E-Mails als gelesen.
</div>

Ordner hinzufügen
<div style="margin-left: 40px">
fügt dem E-Mail-Konto einen Ordner bzw. dem Ordner einen Unterordner hinzu. Da es sich um ein IMAP-Konto handelt, befindet sich ein so angelegter (Unter-)Ordner auf dem E-Mail-Server und ist damit auch auf anderen Geräten sichtbar und benutzbar, auf denen dieses Konto eingerichtet wurde.
</div>

Ordner umbenennen
<div style="margin-left: 40px">
öffnet ein kleines Fenster, in dem Sie dem betreffenden Ordner einen neuen Namen geben können.
</div>

Ordner löschen
<div style="margin-left: 40px">
löscht nur selbst angelegte Unterordner von Standardordnern. Dazu erhalten Sie eine Sicherheitsabfrage. Beachten Sie, dass die hier gespeicherten E-Mails nach dem Löschen des Ordners unwiederbringlich verloren sind.
</div>
...
[^3]:
    Die Textgröße ist für <img src="{{icon_url}}icon_email_size_up.svg" alt="drawing" width="16"/> auf 13px und für <img src="{{icon_url}}icon_email_size_down.svg" alt="drawing" width="16"/> auf 10px festgelegt.

Mit diesen wenigen Angaben ist Ihnen das Verfassen und Empfangen von E-Mails bereits möglich. Mit dem Button Senden oben links im Bearbeitungsmenü versenden Sie die Mail.

Kommen wir jetzt zu einigen Sonderfunktionen, rechts neben dem Senden-Button:

Als Erstes wäre da das X-Icon mit Abbrechen. Dieses ist selbsterklärend. Beim Klicken wird der Schreib-Vorgang abgebrochen und das Fenster schließt sich. Als Nächstes kommt das Plus-Icon mit Datei hinzufügen. Klicken Sie es an, erhalten Sie den Dateiauswahldialog Ihres Betriebssystems, mit dem Sie eine oder auch mehrere beliebige Dateien an die E-Mail anhängen und versenden. Allerdings lassen sich Dateien auch über Drag&Drop an die E-Mail anhängen. Dazu müssen Sie nur den Dateimanager Ihres Betriebssystems mit dem entsprechenden Ordner öffnen und so auf dem Bildschirm arrangieren, dass er sich neben dem E-Mail-Fenster befindet. Dann "ziehen" Sie einfach die Datei mit gedrückter linker Maustaste auf eine beliebige Stelle im E-Mail-Fenster.
...
Erwartungsgemäß löschen Sie darüber markierte E-Mails einzeln oder in Gruppen. Zur Auswahl der Mails stehen Ihnen alle bereits besprochenen Funktionen zur Verfügung. Mit dem Löschen wird die E-Mail lediglich in den Ordner Papierkorb, Mülleimer o.ä. verschoben. Um sie endgültig zu löschen (vgl. [Die Tabelle](ea_Felamimail.md/#die-tabelle)), müssen Sie sie in diesem Ordner erneut markieren und löschen. Wollen Sie den gesamten Mülleimer leeren, geht das über das Kontextmenü, das hier den eigens dafür eingerichteten Menüpunkt Ordner leeren enthält.

### Antworten

Die Funktion Antworten nimmt eine geöffnete oder markierte E-Mail und beantwortet sie – im Unterschied zur Funktion Allen antworten – nur an den eigentlichen Absender. Dabei wird, wie schon weiter oben erwähnt, der Text des Absenders als Zitat übernommen.

### Allen antworten

Allen antworten versendet die Antwort-E-Mail nicht nur an den Absender, sondern auch an eventuelle weitere Empfänger der zu beantwortenden bzw. "zitierten" Ursprungs-E-Mail. Vom Absender in Kopie oder Blindkopie gesetzte Empfänger erhalten die Antwort-E-Mail jeweils wieder als Kopie oder Blindkopie.
...
Nachricht drucken öffnet das Drucker-Auswahlmenü Ihres Betriebssystems; Sie können die Nachricht entweder auf einem Drucker, einem Faxgerät oder beispielsweise einem PDF-Simulator ausdrucken – je nachdem, welche Drucker Ihr System anbietet.
