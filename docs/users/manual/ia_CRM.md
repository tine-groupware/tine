...
Wollen Sie weitere mit dem Lead verbundene Kontakte sehen, rufen Sie die Leadmaske auf (mittels Doppelklick auf den Lead oder Markierung und Button Lead bearbeiten).
...
Lead löschen: entfernt einen markierten Lead, allerdings erst nach Bestätigung.

Drucke Seite: öffnet Ihren browserinternen Druckdialog und erzeugt eine Tabelle wie auf dem Bildschirm, standardmäßig im DIN-A4-Querformat.

Nachricht verfassen: Diese Funktion (ebenfalls nur aktiv bei Markierung eines Leads) öffnet den Dialog Ihres E-Mail-Clients zum Verfassen einer E-Mail. Dabei wird als Absender die E-Mail-Adresse vorgegeben, die als Ihr Standard-E-Mail-Konto im System eingestellt ist (siehe [Benutzerspezifische Einstellungen](na_Benutzereinstellungen.md)); Sie können sie jedoch per Pulldown ändern. Als Adressaten der E-Mail werden alle im Lead erfassten Kontaktadressen vorgegeben, und im Betreff erscheint die Bezeichnung des Leads.

Lead exportieren: Hier stehen zwei Datenformate zur Verfügung, ODS und XLSX (die Tabellenformate von OpenOffice/LibreOffice bzw. Microsoft Excel) beide erzeugen jeweils zwei Tabellen in einer Datei: Overview ist ein komplexes Tableau mit Auswertungen aller verfügbaren Daten, sowohl numerisch als auch grafisch. Leads ist, genau wie in der oben beschriebenen CSV-Datei, eine Tabelle mit den 1:1 ausgelesenen Werten der Datenbank.

Lead importieren: Sie können auch Leads importieren, wenn die Daten im CSV Format vorliegen.
