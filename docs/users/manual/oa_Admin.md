...
Starten Sie {{ branding.title }}. Natürlich müssen Sie sich jetzt als ein Benutzer anmelden, der Administratorrechte hat, denn sonst (siehe oben!) wird Ihnen das folgende Menü gar nicht angezeigt. Rufen Sie über den Reiter ganz links {{ branding.title }} den Menüpunkt Admin auf (wenn er nicht schon als Reiter angezeigt wird). Sie erhalten links eine Reihe von Menüpunkten, denen wir jetzt von oben nach unten jeweils einen Unterabschnitt widmen werden.
...
![Abbildung: Die Benutzertabelle in der Admin-Anwendung]({{ img_url_tablet }}Administration/1_administration_benutzertabelle_light.png#only-light){.tablet-img}
![Abbildung: Die Benutzertabelle in der Admin-Anwendung]({{ img_url_tablet }}Administration/1_administration_benutzertabelle_dark.png#only-dark){.tablet-img}
...
![Abbildung: Eine Auswahl an Spalten, die in der Benutzertabelle angezeigt werden können]({{ img_url_tablet }}Administration/2_administration_spaltenauswahl_light.png#only-light){.tablet-img}
![Abbildung: Eine Auswahl an Spalten, die in der Benutzertabelle angezeigt werden können]({{ img_url_tablet }}Administration/2_administration_spaltenauswahl_dark.png#only-dark){.tablet-img}
...
Machen wir weiter mit einzelnen Spalten der Tabellenansicht, und hier dem Bildschirmnamen. Das ist die Bezeichnung, unter welcher Sie als eingeloggter Benutzer am Bildschirm angezeigt werden. Sie finden ihn immer ganz oben rechts, direkt unter der Begrenzung des Browserfensters -- oder, wenn Sie {{ branding.title }} im Vollbildmodus betreiben -- unter dem Bildschirmrand, links neben dem Knopf zum Abmelden.
...
Die weiteren Felder sind Statusmeldungen: Zuletzt eingeloggt um liefert Datum und Uhrzeit, Letzter Login von die IP-Adresse des Internet-Zugangs, von dem aus der Benutzer zuletzt Zugriff auf {{ branding.title }} hatte.

!!! note "Anmerkung"
    Sollte Ihnen dort jetzt eine IP-Adresse angezeigt werden, die Ihnen unbekannt vorkommt, weil sie nicht derjenigen entspricht, mit der Ihr PC gerade ins Internet geht – es könnte auch Ihr mobiles Gerät gewesen sein, wenn es über ActiveSync an {{ branding.title }} angebunden ist. Dazu weiter unten mehr.
...
![Abbildung: Die Bearbeitungsmaske zum Anlegen eines neuen Benutzers]({{ img_url_tablet }}Administration/3_administration_benutzer_neu_light.png#only-light){.tablet-img}
![Abbildung: Die Bearbeitungsmaske zum Anlegen eines neuen Benutzers]({{ img_url_tablet }}Administration/3_administration_benutzer_neu_dark.png#only-dark){.tablet-img}
...
!!! note "Anmerkung"
    ...
    * Wem erlaube ich überhaupt Zugang zu {{ branding.title }} (z.B. in Hinblick auf externe Benutzer...)?
...
Die Tatsache, ob ein Benutzer in einem Adressbuch gespeichert ist oder nicht, hat weitreichende Folgen: Nur in Adressbüchern registrierte Benutzer können in den verschiedensten {{ branding.title }}-Anwendungen ausgewählt werden, die anderen sind nicht sichtbar!

Ein Anwendungsfall könnte bspw. auch sein, dass Sie zwei (oder mehr) verschiedene interne Adressbücher anlegen wollen, deren Benutzer sich gegenseitig nicht sehen sollen. Sie legen dann zwei Gruppen an, die jeweils nur Zugriff auf eines der beiden Adressbücher haben. Damit haben Sie zwei Benutzergruppen, die sich zwar alle an {{ branding.title }} anmelden, sich aber gegenseitig nicht sehen können.
...
![Abbildung: Das Zuweisen eines neuen Benutzers zu einer Gruppe]({{ img_url_tablet }}Administration/4_administration_benutzer_gruppe_light.png#only-light){.tablet-img}
![Abbildung: Das Zuweisen eines neuen Benutzers zu einer Gruppe]({{ img_url_tablet }}Administration/4_administration_benutzer_gruppe_dark.png#only-dark){.tablet-img}
...
Fileserver, IMAP, SMTP – Diese Reiter sind bei einer "stand-alone"-Installation von {{ branding.title }} ausgegraut. Sollte Ihre {{ branding.title }}-Installation jedoch mit einem File- und/oder E-Mail-Server verbunden sein, dann haben Sie über diese Reiter die Möglichkeit, bestimmte Eigenschaften dieser Server einzusehen oder zu verändern. Die Beschreibung dieser Möglichkeiten würde allerdings den Rahmen des vorliegenden Handbuches sprengen.
...
![Abbildung: Die Bearbeitungsmaske zum Editieren eines Benutzerkontos]({{ img_url_tablet }}Administration/5_administration_benutzer_editieren_light.png#only-light){.tablet-img}
![Abbildung: Die Bearbeitungsmaske zum Editieren eines Benutzerkontos]({{ img_url_tablet }}Administration/5_administration_benutzer_editieren_dark.png#only-dark){.tablet-img}
...
![Abbildung: Verwaltung der Gruppen in {{ branding.title }}.]({{ img_url_tablet }}Administration/6_administration_gruppen_light.png#only-light){.tablet-img}
![Abbildung: Verwaltung der Gruppen in {{ branding.title }}.]({{ img_url_tablet }}Administration/6_administration_gruppen_dark.png#only-dark){.tablet-img}
...
Wenn {{ branding.title }} in einer unveränderten Erstinstallation vorliegt, dann dürften Sie jetzt zwei Gruppen sehen, nämlich Users und Administrators. Diese beiden Gruppen sind normalerweise in jeder {{ branding.title }}-Installation enthalten. Natürlich können in Ihrer Installation auch noch viele weitere Gruppen enthalten sein. Damit Sie sich in der Tabellenansicht unter evtl. sehr vielen Gruppen die richtigen anzeigen lassen können, gibt es auch hier eine Suchfilter-Funktion: Eine im Suchfeld eingegebene Zeichenfolge filtert die Gruppen, allerdings nur über das Feld Name.
...
![Abbildung: Die Maske zum Bearbeiten einer Benutzergruppe]({{ img_url_tablet }}Administration/7_administration_gruppen_editieren_light.png#only-light){.tablet-img}
![Abbildung: Die Maske zum Bearbeiten einer Benutzergruppe]({{ img_url_tablet }}Administration/7_administration_gruppen_editieren_dark.png#only-dark){.tablet-img}
...
Kommen wir nun zu einem der wichtigsten Teile des Administrierens einer Groupware, dem Definieren von "Rollen". Wie wir weiter oben schon besprochen haben, dienen Rollen dem Zuweisen von "Rechten" zur Benutzung der verschiedenen Programmteile von {{ branding.title }} zu bestimmten Benutzern oder Benutzergruppen - also welche Rolle ein Benutzer im Gesamtsystem spielen darf. Schauen wir uns das praktisch an:
...
![Abbildung: Die Maske zum Zuweisen von Benutzern oder Gruppen zu einer Rolle (Reiter "Mitglieder")".]({{ img_url_tablet }}Administration/8_administration_rolle_editieren_light.png#only-light){.tablet-img}
![Abbildung: Die Maske zum Zuweisen von Benutzern oder Gruppen zu einer Rolle (Reiter "Mitglieder")".]({{ img_url_tablet }}Administration/8_administration_rolle_editieren_dark.png#only-dark){.tablet-img}
...
![Abbildung: Die Maske zum Definieren der Rechte einer Rolle (Reiter "Rechte")".]({{ img_url_tablet }}Administration/9_administration_rolle_rechte_editieren_light.png#only-light){.tablet-img}
![Abbildung: Die Maske zum Definieren der Rechte einer Rolle (Reiter "Rechte")".]({{ img_url_tablet }}Administration/9_administration_rolle_rechte_editieren_dark.png#only-dark){.tablet-img}
...
2. Admin – die Anwendung zur Administration der gesamten Groupware
    ...
    * Computer ansehen - Darf der Benutzer bei Verwendung der Samba-Integration die Computerkonten der Workstations sehen (siehe [Anmerkung](oa_Admin.md/#anmerkung))
    * Computer verwalten - Darf der Benutzer bei Verwendung der Samba-Integration die Computerkonten der Workstations anlegen, bearbeiten und löschen (siehe [Anmerkung](oa_Admin.md/#anmerkung))
...
14. Tinebase
    ...
    * Wartung – Darf der Benutzer die {{ branding.title }} Installation im Wartungsmodus verwenden?
15. Zeiterfassung
...
!!! note "Anmerkung"
    ...
    Bei einer Integration von {{ branding.title }} in ein Active Directory[^2] können Sie an dieser Stelle die Konten für Computer verwalten, die sich am Active Directory anmelden dürfen. {{ branding.title }} unterstützt die Integration in Samba[^3] oder Microsoft-basierte Active-Directory-Umgebungen.
...
![Abbildung: Die einzelnen Anwendungen von {{ branding.title }}]({{ img_url_tablet }}Administration/10_administration_anwendungen_light.png#only-light){.tablet-img}
![Abbildung: Die einzelnen Anwendungen von {{ branding.title }}]({{ img_url_tablet }}Administration/10_administration_anwendungen_dark.png#only-dark){.tablet-img}
...
![Abbildung: Einstellungen der Admin-Anwendung]({{ img_url_tablet }}Administration/11_administration_admin_einstellung_light.png#only-light){.tablet-img}
![Abbildung: Einstellungen der Admin-Anwendung]({{ img_url_tablet }}Administration/11_administration_admin_einstellung_dark.png#only-dark){.tablet-img}
...
![Abbildung: Die Maske zum Anlegen einer neuen Ressource für die Kalender-Anwendung]({{ img_url_tablet }}Administration/13_administration_kalender_ressource_neu_light.png#only-light){.tablet-img}
![Abbildung: Die Maske zum Anlegen einer neuen Ressource für die Kalender-Anwendung]({{ img_url_tablet }}Administration/13_administration_kalender_ressource_neu_dark.png#only-dark){.tablet-img}
...
![Abbildung: Das Zuweisen von Zugriffsrechten zu einer angelegten Ressource]({{ img_url_tablet }}Administration/14_administration_kalender_ressource_rechte_light.png#only-light){.tablet-img}
![Abbildung: Das Zuweisen von Zugriffsrechten zu einer angelegten Ressource]({{ img_url_tablet }}Administration/14_administration_kalender_ressource_rechte_dark.png#only-dark){.tablet-img}
...
Diese Benachrichtigungen müssen in der Grundkonfiguration von {{ branding.title }} aktiviert werden. Wenden Sie sich hierfür an Ihren Administrator.
...
![Abbildung: Die Standardeinstellungen der CRM-Anwendung: Lead-Status, -Quelle und -Typ]({{ img_url_tablet }}Administration/15_administration_crm_einstellungen_light.png#only-light){.tablet-img}
![Abbildung: Die Standardeinstellungen der CRM-Anwendung: Lead-Status, -Quelle und -Typ]({{ img_url_tablet }}Administration/15_administration_crm_einstellungen_dark.png#only-dark){.tablet-img}
...
![Abbildung: Die Maske zum Verwalten der möglichen Lead-Status der CRM-Anwendung]({{ img_url_tablet }}Administration/16_administration_crm_lead_status_light.png#only-light){.tablet-img}
![Abbildung: Die Maske zum Verwalten der möglichen Lead-Status der CRM-Anwendung]({{ img_url_tablet }}Administration/16_administration_crm_lead_status_dark.png#only-dark){.tablet-img}
...
![Abbildung: Die Einstellungen der HumanResources-Anwendung]({{ img_url_tablet }}Administration/17_administration_hr_einstellungen_light.png#only-light){.tablet-img}
![Abbildung: Die Einstellungen der HumanResources-Anwendung]({{ img_url_tablet }}Administration/17_administration_hr_einstellungen_dark.png#only-dark){.tablet-img}
...
![Abbildung: Die Einstellungen der Sales-Anwendung]({{ img_url_tablet }}Administration/18_administration_sales_einstellungen_light.png#only-light){.tablet-img}
![Abbildung: Die Einstellungen der Sales-Anwendung]({{ img_url_tablet }}Administration/18_administration_sales_einstellungen_dark.png#only-dark){.tablet-img}
...
* Eigene Währung ist selbsterklärend. Hier stellt man die Währung ein, in der {{ branding.title }} fakturieren soll.[^4]
...
* Product Number Zero Fill: Gibt an wie viele Ziffern {{ branding.title }} für eine Produktnummer verwendet werden.
...
<!-- SCREENSHOT -->
![Abbildung: Die Profil-Einstellungen der {{ branding.title }}base-Anwendung.]({{ img_url_desktop }}Administration/19_administration_tinebase_einstellungen_light.png#only-light){.desktop-img}
![Abbildung: Die Profil-Einstellungen der {{ branding.title }}base-Anwendung.]({{ img_url_desktop }}Administration/19_administration_tinebase_einstellungen_dark.png#only-dark){.desktop-img}
![Abbildung: Die Profil-Einstellungen der {{ branding.title }}base-Anwendung.]({{ img_url_tablet }}Administration/19_administration_tinebase_einstellungen_light.png#only-light){.tablet-img}
![Abbildung: Die Profil-Einstellungen der {{ branding.title }}base-Anwendung.]({{ img_url_tablet }}Administration/19_administration_tinebase_einstellungen_dark.png#only-dark){.tablet-img}
![Abbildung: Die Profil-Einstellungen der {{ branding.title }}base-Anwendung.]({{ img_url_mobile }}Administration/19_administration_tinebase_einstellungen_light.png#only-light){.mobile-img}
![Abbildung: Die Profil-Einstellungen der {{ branding.title }}base-Anwendung.]({{ img_url_mobile }}Administration/19_administration_tinebase_einstellungen_dark.png#only-dark){.mobile-img}

Tinebase: Unter dem einzigen Reiter, Profilinformation finden Sie eine Reihe von Feldern für Adressdaten und jeweils zwei zugehörige Checkbuttons: Lesen und Bearbeiten. Hier können Sie nach Bedarf, zusätzlich zu den Standardfeldern, die möglichen Dateneingaben in der Adressdatenbank erweitern. Die zusätzlichen Felder könnten insbesondere dann Bedeutung erlangen, wenn Sie Adressdaten aus anderen Programmen, wie z.B. Microsoft Outlook in {{ branding.title }} einlesen. Outlook hat sehr viele mögliche Felder in der Adressdatenbank. Ob diese belegt sind, sehen Sie, wenn Sie sich die CSV-Tabelle anschauen, die Outlook beim Auslesen erzeugt. Schlagen Sie für nähere Erläuterungen hierzu bitte im [Adressverwaltung - Kontakte importieren](ba_Addressbook.md/#kontakte-importieren) nach!
...
"Container" werden die einzelnen Datenbanken genannt, die jeweils über die Anwendungen von {{ branding.title }} erzeugt werden können. Zum Beispiel können das die schon allseits bekannten Adressbücher sein, aber auch Kalender, Aufgabenlisten, Zeitkonten usw.
...
![Abbildung: Eine Übersicht der angelegten Container (gefiltert)]({{ img_url_tablet }}Administration/24_administration_container_light.png#only-light){.tablet-img}
![Abbildung: Eine Übersicht der angelegten Container (gefiltert)]({{ img_url_tablet }}Administration/24_administration_container_dark.png#only-dark){.tablet-img}
...
![Abbildung: Die Maske zur Container-Bearbeitung enthält nicht nur Angaben zum Datenmodell, sondern auch die Benutzer- oder Gruppenzuweisung sowie die zugehörige Vergabe von Berechtigungen.]({{ img_url_tablet }}Administration/25_administration_container_editieren_light.png#only-light){.tablet-img}
![Abbildung: Die Maske zur Container-Bearbeitung enthält nicht nur Angaben zum Datenmodell, sondern auch die Benutzer- oder Gruppenzuweisung sowie die zugehörige Vergabe von Berechtigungen.]({{ img_url_tablet }}Administration/25_administration_container_editieren_dark.png#only-dark){.tablet-img}
...
Bis auf die persönlichen Kalender-Container (dort gibt es zwei zusätzliche Einstellungen, Frei/Belegt und Privat, die im [Kalender - Termin hinzufügen/bearbeiten](da_Calendar.md/#termin-hinzufugenbearbeiten) näher erläutert werden) sind die angebotenen Berechtigungen immer dieselben:
...
* Sync: Der Benutzer darf die Datensätze per ActiveSync, WebDAV, CalDAV oder CardDAV auf sein Smartphone oder Computer synchronisieren. Mit dieser Einstellung können Sie (mit Einschränkungen) verhindern, dass Daten aus {{ branding.title }} einfach über diese Funktion heraus kopiert werden können.
...
![Abbildung: Die Bearbeitungsmaske zum Anlegen eines neuen Containers]({{ img_url_tablet }}Administration/26_administration_container_neu_light.png#only-light){.tablet-img}
![Abbildung: Die Bearbeitungsmaske zum Anlegen eines neuen Containers]({{ img_url_tablet }}Administration/26_administration_container_neu_dark.png#only-dark){.tablet-img}
...
<!--Tags,gemeinsame-->
Tags sind die kleinen Markierungen an beliebigen Datensätzen, die es uns erlauben, bestimmte Daten aus einer großen Menge herauszufiltern. Dabei unterscheidet {{ branding.title }} in Gemeinsame Tags und Persönliche Tags. Persönliche Tags können Sie als Benutzer in beliebigen Teilen des Programms setzen, ohne über Administratorrechte zu verfügen. Wir haben darüber bereits im [Adressverwaltung](ba_Addressbook.md) gesprochen. Hier geht es jedoch um die gemeinsamen Tags, die für ganze Benutzergruppen eingerichtet werden und die nur ein Administrator verwalten darf.
...
![Abbildung: Eine Liste der gemeinsamen Tags, die jeder Nutzer verwenden kann]({{ img_url_tablet }}Administration/21_administration_gemeinsame_tags_light.png#only-light){.tablet-img}
![Abbildung: Eine Liste der gemeinsamen Tags, die jeder Nutzer verwenden kann]({{ img_url_tablet }}Administration/21_administration_gemeinsame_tags_dark.png#only-dark){.tablet-img}
...
![Abbildung: Das Zuweisen von Benutzern oder Gruppen zu einem gemeinsamen Tag und die Definition der entsprechenden Benutzerrechte]({{ img_url_tablet }}Administration/22_administration_gemeinsame_tags_rechte_light.png#only-light){.tablet-img}
![Abbildung: Das Zuweisen von Benutzern oder Gruppen zu einem gemeinsamen Tag und die Definition der entsprechenden Benutzerrechte]({{ img_url_tablet }}Administration/22_administration_gemeinsame_tags_rechte_dark.png#only-dark){.tablet-img}
...
![Abbildung: Die Einstellung, in welchen Anwendungen ein gemeinsamer Tag anwendbar und gültig sein soll]({{ img_url_tablet }}Administration/23_administration_gemeinsame_tags_kontexte_light.png#only-light){.tablet-img}
![Abbildung: Die Einstellung, in welchen Anwendungen ein gemeinsamer Tag anwendbar und gültig sein soll]({{ img_url_tablet }}Administration/23_administration_gemeinsame_tags_kontexte_dark.png#only-dark){.tablet-img}
...
![Abbildung: Eine Übersicht aller im System angelegten Zusatzfelder]({{ img_url_tablet }}Administration/27_administration_zusatzfelder_light.png#only-light){.tablet-img}
![Abbildung: Eine Übersicht aller im System angelegten Zusatzfelder]({{ img_url_tablet }}Administration/27_administration_zusatzfelder_dark.png#only-dark){.tablet-img}
...
![Abbildung: Das Bearbeitungsfenster zum Anlegen eines neuen Zusatzfeldes]({{ img_url_tablet }}Administration/28_administration_zusatzfelder_neu_light.png#only-light){.tablet-img}
![Abbildung: Das Bearbeitungsfenster zum Anlegen eines neuen Zusatzfeldes]({{ img_url_tablet }}Administration/28_administration_zusatzfelder_neu_dark.png#only-dark){.tablet-img}
...
Anwendung ist als Pulldown ausgeführt; es bietet Ihnen die relevanten {{ branding.title }}-Anwendungen an, in deren Datencontainern Sie Zusatzfelder definieren können.
...
* Datensatz: Hier können Sie eine Anwendung und einen Datentyp auswählen. Zum Beispiel Adressbuch und Kontakt. In der Oberfläche kann der Benutzer dann in diesem Zusatzfeld über ein Pulldown einen Kontakt aus dem Adressbuch auswählen. Sie kennen diese Art der Bestimmung von Datenfeldinhalten schon aus {{ branding.title }}, wo sie in vielen Anwendungen für die Bildung von Verknüpfungen genutzt wird.
...
Die Funktion Kontingents-Nutzung bietet eine Übersicht darüber, wieviel Datenspeicher die unterschiedlichen Anwendungen von {{ branding.title }} nutzen.
...
![Abbildung: Übersicht aller ActiveSync Geräte]({{ img_url_tablet }}Administration/30_administration_activesync_devices_light.png#only-light){.tablet-img}
![Abbildung: Übersicht aller ActiveSync Geräte]({{ img_url_tablet }}Administration/30_administration_activesync_devices_dark.png#only-dark){.tablet-img}
...
!!! tip "Tipp"
    Sollte sich ein Gerät nicht mehr mit {{ branding.title }} über ActiveSync synchronisieren, dann entfernen Sie das Gerät mit Klick auf Sync Gerät löschen aus dieser Liste. Das Gerät meldet sich daraufhin neu bei {{ branding.title }} an, und die Synchronisation funktioniert wieder.
...
![Abbildung: Angaben eines ActiveSync Gerätes bearbeiten.]({{ img_url_tablet }}Administration/31_administration_activesync_devices_editieren_light.png#only-light){.tablet-img}
![Abbildung: Angaben eines ActiveSync Gerätes bearbeiten.]({{ img_url_tablet }}Administration/31_administration_activesync_devices_editieren_dark.png#only-dark){.tablet-img}
...
* Der Eigentümer ist der Eigentümer des Gerätes. Diese Angabe ist immer vorhanden, da sie aus der Kontobezeichnung des {{ branding.title }}-Benutzers übernommen wird.
...
![Abbildung: Das Zugriffslog]({{ img_url_tablet }}Administration/20_administration_zugriffslog_light.png#only-light){.tablet-img}
![Abbildung: Das Zugriffslog]({{ img_url_tablet }}Administration/20_administration_zugriffslog_dark.png#only-dark){.tablet-img}
...
<!--ActiveSync-->
Was passiert, wenn wir die gesetzten Filter entfernen? Gehen Sie probehalber bitte einmal auf die beiden --Symbole oben links über dem Tabellenkopf und klicken sie weg – die Verfahrensweise ist Ihnen wahrscheinlich schon vom [Adressverwaltung](ba_Addressbook.md) bekannt. Gehen Sie jetzt auf Suche starten (d.h. aktivieren Sie eine neue Ansicht, diesmal ohne Filter): Möglicherweise sehen Sie jetzt wesentlich mehr Logins. Nicht nur weil Sie die Zeitbeschränkung (letzte Woche) aufgehoben haben, sondern auch, weil Logins von anderen Geräten angezeigt werden, sofern es welche gab. Schauen Sie ganz rechts auf die Spalte Client-Typ! Sehen Sie Einträge mit der Bezeichnung TineActiveSync? Damit hat es folgende Bewandnis: Wie Sie vielleicht schon wissen, ist ein großer Vorteil von {{ branding.title }} gegenüber anderen im Markt befindlichen Systemen dieser Art die einfache Einbindung von Handheld-Devices, wie Smartphones und Tablets. Dabei ist es gleichgültig, welches Betriebssystem  das Gerät hat (Android, iOS, Windows Mobile/Phone, Blackberry), sofern es nur eine sog. ActiveSync-Funktion aufweist. Diese Funktion ist ursprünglich ein Microsoft-Patent (in manchen Staaten, wie z.B. USA, ist es das heute noch, hier gilt es besondere Bedingungen beim Einsatz von {{ branding.title }} zu beachten!) und als solches quasi zum Industriestandard geworden. Die ActiveSync-Schnittstelle ist sehr robust und garantiert eine fehlerfreie wechselseitige Datensynchronisation von E-Mail, Kontakten und Kalenderdaten zwischen {{ branding.title }} und dem Handheld-Device, ohne dass auf diesem irgendwelche Apps installiert werden müssten. Ganz nebenbei: Wenn Sie auf diesem Wege Ihre sensiblen Kontakt- und Termindaten auf einem {{ branding.title }}-System abspeichern, anstatt bei Google (wenn Sie ein Android-Smartphone nutzen) oder Apple (beim iPhone), entgehen Sie auch der Ausspähung und potenziellen Weiterleitung Ihrer Daten durch die genannten Unternehmen an die Geheimdienste und andere Behörden.
...
![Abbildung: Informationen zum Server]({{ img_url_tablet }}Administration/29_administration_serverinfo_light.png#only-light){.tablet-img}
![Abbildung: Informationen zum Server]({{ img_url_tablet }}Administration/29_administration_serverinfo_dark.png#only-dark){.tablet-img}
...
Hier können Sie unter anderen die in {{ branding.title }} genutzten E-Mailkonten bearbeiten. Dies wurde im E-Mail Kapitel unter dem [E-Mail - Das Bearbeitungsmenü](ea_Felamimail.md/#das-bearbeitungsmenu) schon besprochen. Für weitere Information navigieren Sie dort hin.
...
Mit dieser Option können Sie ein weiteres E-Mail-Konto erstellen, welches über Benutzer einem individuellen User zugeteilt werden kann. Das neue E-Mail-Konto wird dem ausgewählten Benutzer nach einem "Refresch" von {{ branding.title }} zur Verfügung stehen.

* Persönliches externes E-Mail-Konto:

Mit Persönliches externes E-Mail-Konto kann man externe E-Mail-Konten zu {{ branding.title }} hinzufügen[^5]. Auch hier definiert Benutzer, welchem Benutzer dieses Konto zugeordnet werden soll.
