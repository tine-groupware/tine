...
Um die OnlyOffice-Integration zu nutzen, müssen Sie sich im ersten Schritt mit der Firma Metaways oder einen ihrer Partner in Verbindung setzen, um Ihre Subscription der {{ branding.title }}-Installation zu erweitern. Wenn dies geschehen ist, kann Ihre OnlyOffice-Installation an {{ branding.title }} angebunden werden.

## Einleitung

Sofern Ihre OnlyOffice-Installation in {{ branding.title }} angebunden ist, sollten Sie nun von dieser Gebrauch machen können.

Eine tiefgehende Gebrauchsanweisung wie OnlyOffice zu nutzen ist, finden Sie direkt beim Hersteller. Im Folgenden soll aufgezeigt werden, wo OnlyOffice integriert wurde und wie es von {{ branding.title }} aus zu benutzen ist.
...
Die OnlyOffice-Integration bietet die Möglichkeit, dass mehrere Personen gleichzeitig an einem Dokument arbeiten. Dies ist jedoch nur konfliktfrei möglich, wenn alle Personen die Datei vom Browser aus benutzen. (Da {{ branding.title }} auch webDAV unterstützt, wäre es z.B. möglich die iPad-App von OnlyOffice zu nutzen, um ein Dokument zu bearbeiten.)
...
* Bild aus dem Speicher: Bietet die Integration eines Bildes direkt aus dem Dateimanager von {{ branding.title }}.
