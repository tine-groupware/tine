...
describe('MainScreen', () => {
    test('Grid', async () => {
        await lib.makeScreenshot(page, 'screenshots/StandardBedienhinweise/5_standardbedienhinweise_email.png');
...
describe('editDialog', () => {
    ...
    test('open editDialog', async () => {
        ...
        await lib.makeScreenshot(newPage, 'screenshots/EMail/4_email_neues_konto.png');
    });
    test('imap', async () => {
        await expect(newPage).toClick('span', {text: 'IMAP'});
        await newPage.waitForTimeout(1000);
        await expect(newPage).toFill('input[name=host]', 'mail.tine20.net');
        await lib.makeScreenshot(newPage, 'screenshots/EMail/5_email_neues_konto_imap.png');
    });
    test('smtp', async () => {
        await expect(newPage).toClick('span', {text: 'SMTP'});
        await newPage.waitForTimeout(1000);
        await expect(newPage).toFill('input[name=smtp_hostname]', 'mail.tine20.net');
        await lib.makeScreenshot(newPage, 'screenshots/EMail/6_email_neues_konto_smtp.png');
    });
    test('other settings', async () => {
        await newPage.waitForTimeout(1000);
        await expect(newPage).toClick('span', {text: 'Andere Einstellungen'});
        await lib.makeScreenshot(newPage, 'screenshots/EMail/7_email_neues_konto_andere.png');
        await newPage.close()
    })
});
...
describe('context menu', () => {
    ...
    test('open context menu', async () => {
        ...
        await lib.makeScreenshot(page, 'screenshots/EMail/8_email_konto_kontextmenu.png');
    });
    test.skip('absence note', async () => {
        await page.hover('.x-menu-item-icon.action_email_replyAll');
        await lib.makeScreenshot(page, 'screenshots/EMail/18_email_server_kontextmenu.png');
        ...
        await lib.makeScreenshot(newPage, 'screenshots/EMail/10_email_abwesenheitsnotiz.png');
        await newPage.close();
    });
    test.skip('add mail filter', async () => {
        ...
        await popup.close();
        await lib.makeScreenshot(newPage, 'screenshots/EMail/11_email_empfangsfilter.png');
        await newPage.close();
    });
    test('create folder', async () => {
        await expect(page).toClick('span', {text: 'Posteingang', button: 'right'});
        await page.waitForTimeout(1000);
        await lib.makeScreenshot(page, 'screenshots/EMail/9_email_ordner_kontextmenu.png');
...
describe('filterBar', () => {
    ...
    test('details display', async () => {
        ...
        await lib.makeScreenshot(page, 'screenshots/StandardBedienhinweise/7_standardbedienhinweise_email_suchoptionen.png');
        await page.keyboard.press('Escape');
    });
    ...
    test('operator for filter', async () => {
        ...
        await lib.makeScreenshot(page, 'screenshots/StandardBedienhinweise/11_standardbedienhinweise_email_suchfilter_operatoren.png');
    });
    ...
    test('save filter as favoriten', async () => {
        ...
        await lib.makeScreenshot(page, 'screenshots/EMail/15_allgemein_email_filter_favoriten.png');
        await lib.makeScreenshot(page, 'screenshots/StandardBedienhinweise/15_standardbedienhinweise_email_filter_favoriten.png');
        await page.keyboard.press('Escape');
        await page.waitForTimeout(500);
        try {
