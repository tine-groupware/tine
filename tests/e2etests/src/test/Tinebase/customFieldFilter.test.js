...
describe.skip('Mainpage', () => {
    test('create customFieldKeyField', async () => {
        ...
        await expectPuppeteer(editDialog).toClick('.x-combo-list-item', {text:'Crm',visible:true});
        await editDialog.click('input[name=type]');
        await expectPuppeteer(editDialog).toClick('.x-combo-list-item', {text:'Schlüsselfeld',visible:true});
        await expectPuppeteer(editDialog).toClick('button', {text:'Datenspeicher konfigurieren'});
        ...
        await expectPuppeteer(editDialog).toClick('button', {text: 'Ok'});
        await new Promise(r => setTimeout(r, 1000)); //wait to create customField
    ...
    test.skip('filter null customField-keyField', async () => {
        ...
        await expectPuppeteer(page).toClick('.x-combo-list-item', {text:'testcustomField-keyField',visible:true});
        await expectPuppeteer(page).toClick('button', {text:'Suche starten',visible:true});
