...
Promise.all([Tine.Tinebase.appMgr.isInitialised('Sales'),
    ...
    ['Offer', 'Order', 'Delivery', 'Invoice'].forEach((type) => {
        ...
        Ext.ux.ItemRegistry.registerItem(`Sales-Document_${type}-GridPanel-ActionToolbar-leftbtngrp`, Ext.apply(new Ext.Button(action), medBtnStyle), 39)
        Ext.ux.ItemRegistry.registerItem(`Sales-Document_${type}-editDialog-Toolbar`, Ext.apply(new Ext.Button(action), medBtnStyle), 100)
    })
})
