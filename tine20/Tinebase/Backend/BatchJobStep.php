...
class Tinebase_Backend_BatchJobStep extends Tinebase_Backend_Sql
{
    ...
    public function addInData(string $parentStepId, Tinebase_BatchJob_InOutData $inData): void
    {
        ($db = $this->getAdapter())->update($this->getPrefixedTableName(), [
            Tinebase_Model_BatchJobStep::FLD_TO_PROCESS => new Zend_Db_Expr('JSON_ARRAY_APPEND(' . Tinebase_Model_BatchJobStep::FLD_TO_PROCESS . ', ' . $db->quote($inData->getId()) . ')'),
        ], $db->quoteIdentifier(Tinebase_Model_BatchJobStep::FLD_PARENT_ID) . ' = ' . $db->quote($parentStepId)
            . ' AND NOT JSON_CONTAINS_PATH(' . Tinebase_Model_BatchJobStep::FLD_IN_DATA . ', "one", "$.' . $inData->getId() . '")');

        $db->update($this->getPrefixedTableName(), [
            Tinebase_Model_BatchJobStep::FLD_IN_DATA => new Zend_Db_Expr('JSON_INSERT(' . Tinebase_Model_BatchJobStep::FLD_IN_DATA . ', "$.' . $inData->getId() . '", ' . $db->quote(json_encode($inData->getData())) . ')'),
        ], $db->quoteIdentifier(Tinebase_Model_BatchJobStep::FLD_PARENT_ID) . ' = ' . $db->quote($parentStepId));
    }
