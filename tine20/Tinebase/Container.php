...
class Tinebase_Container extends Tinebase_Backend_Sql_Abstract implements Tinebase_Controller_SearchInterface, Tinebase_Container_Interface
{
     ...
     * @var integer
     */
    public const ACL_CACHE_TIMEOUT = 30;
...
//                    Tinebase_Model_Grants::GRANT_SYNC    => true,
    ...
    public function addGrants($_containerId, $_accountType, $_accountId, array $_grants, $_ignoreAcl = FALSE)
    {
        ...
        $accountId = match ($_accountType) {
            Tinebase_Acl_Rights::ACCOUNT_TYPE_USER => Tinebase_Model_User::convertUserIdToInt($_accountId),
            Tinebase_Acl_Rights::ACCOUNT_TYPE_GROUP => Tinebase_Model_Group::convertGroupIdToInt($_accountId),
            Tinebase_Acl_Rights::ACCOUNT_TYPE_ANYONE => '0',
            Tinebase_Acl_Rights::ACCOUNT_TYPE_ROLE => Tinebase_Model_Role::convertRoleIdToInt($_accountId),
            default => throw new Tinebase_Exception_InvalidArgument('invalid $_accountType'),
        };
    ...
    protected function _resolveRecordClassArgument($_recordClass)
    {
        ...
        if(is_string($_recordClass)) {
            ...
            $ret['recordClass'] = $_recordClass::class;
        } else {
    ...
    public function getContainerByACL($accountId, $recordClass, $grant, $onlyIds = FALSE, $ignoreACL = FALSE)
    {
        ...
        static::addGrantsSql($select, $accountId, $grant);
    ...
    public static function addGrantsSql($_select, $_accountId, $_grant, $_aclTableName = 'container_acl', $_andGrants = FALSE, $joinCallBack = null)
    {
        ...
        if (!in_array('*', $grants)) {
            ...
            if (null === $joinCallBack) {
                $joinCallBack = static::addGrantsSqlCallback(...);
            }
            $iteration = 0;
            ...
            foreach ($grants as $grant) {
                ...
                if ($grant[strlen((string) $grant) - 1] === ')') {
                    $closeBracket = true;
    ...
    public function getSharedContainer($_accountId, $recordClass, $_grant, $_ignoreACL = FALSE, $_andGrants = FALSE)
    {
        ...
        static::addGrantsSql($select, $accountId, $grant, 'container_acl', $_andGrants, self::class . '::addGrantsSqlCallback');
    ...
    public function deleteContainerContents($container, $_ignoreAcl = FALSE)
    {
        ...
        if ($controller) {
            ...
            if (method_exists($controller, 'deleteContainerContents')) {
                ...
                Tinebase_Core::getLogger()->notice(__METHOD__ . '::' . __LINE__
                    . ' No deleteContainerContents defined in controller ' . $controller::class);
            }
    ...
    public function getContainersByApplicationId($_applicationId, $_accountId, $_grants)
    {
        ...
        static::addGrantsSql($select, $accountId, $_grants);
    ...
    public function hasGrant($_accountId, $_containerId, $_grant)
    {
        ...
        try {
            ...
            static::addGrantsSql($select, $accountId, '*');
    ...
    public function getContainerWithGrants(array $_containerIds, $_accountId): array
    {
        ...
        $where[1] = rtrim((string) $where[1], ')') . ') OR container_acl.account_type IS NULL)';
            ...
            foreach ($_grants as $recordGrants) {
                $data = array(
                    ...
                    'account_id' => is_array($recordGrants['account_id']) ?
                        ($recordGrants['account_id']['id'] ?? $recordGrants['account_id']['accountId']) : $recordGrants['account_id'],
     ...
     * @param string $name
     * @param ?string $configId get/save id from/in config if given
     ...
     * @throws Tinebase_Exception_SystemGeneric
     */
    public function createSystemContainer($application,
                                          string $model,
                                          string $name,
                                          ?string $configId = null,
                                          ?Tinebase_Record_RecordSet $grants = null,
                                          ?Tinebase_Config_Abstract $config = null): Tinebase_Model_Container
    {
        $application = ($application instanceof Tinebase_Model_Application)
            ? $application
            : Tinebase_Application::getInstance()->getApplicationById($application);
        $configContainerId = null;

        if ($configId !== null) {
            $configClass = $application->name . '_Config';
            if (! $config && @class_exists($configClass)) {
                $config = call_user_func(array($configClass, 'getInstance'));
            }
            if ($config) {
                $configContainerId = $config->get($configId);
            } else {
                if (Tinebase_Core::isLogLevel(Zend_Log::NOTICE)) Tinebase_Core::getLogger()->notice(__METHOD__ . '::' . __LINE__
        ...
        if (! $systemContainer) {
            ...
            if ($configId !== null && $config) {
                if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) Tinebase_Core::getLogger()->debug(
                    __METHOD__ . '::' . __LINE__ . ' Setting system container config "' . $configId . '" = '
                    . $systemContainer->getId());
     ...
     * @throws Tinebase_Exception_InvalidArgument
     */
    protected function _getExistingContainer(array $props, ?string $configId = null): ?Tinebase_Model_Container
    {
        if ($configId) {
            try {
                /* @var Tinebase_Model_Container $container */
