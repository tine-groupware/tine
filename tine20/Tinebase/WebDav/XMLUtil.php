...
class Tinebase_WebDav_XMLUtil
{
    ...
    static function loadDOMDocument($xml)
    {
        ...
        if (empty($xml))
            throw new \Sabre\DAV\Exception\BadRequest('Empty XML document sent');
        ...
        if ($error = libxml_get_last_error()) {
            libxml_clear_errors();
            throw new \Sabre\DAV\Exception\BadRequest('The request body had an invalid XML body. (message: ' . $error->message . ', errorcode: ' . $error->code . ', line: ' . $error->line . ')');
        }
